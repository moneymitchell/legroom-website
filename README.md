...
- `assets/` holds the background art, the logo source files, and the share image. The logo on the page is inlined into `index.html` from `legroom-lockup-horizontal-charcoal.svg` so the mark and wordmark can be colored separately in CSS. Edit the SVG file, then re-inline it. Everything in it is already optimized. Do not re-export, re-compress, or regenerate anything.
...
- The content block sits at 40% from the top on desktop and 27% on mobile. Both were set by looking at the art, not by formula. The mobile crop has a cloud bank between roughly 26% and 31% of the viewport height and the logo has to clear it. If the background art changes, re-check these.

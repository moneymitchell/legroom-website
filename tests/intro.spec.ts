...
test("a first visit on a desktop plays the intro and then gets out of the way", async ({
  browser,
}) => {
  ...
  // the canvas the gate created is the one that draws
...
test("the lockup lands on the nav's own box, measured", async () => {
  ...
  // 120ms dissolve on a polled screenshot, which a starved software GPU can
  // miss: the box the module laid the element out in when the beat began
  // must be the nav lockup's box to the pixel, and the travel animation's
  // final keyframe must be transform: none, so that box is where it ends.
...
for (const [label, act] of [
  ...
  test(`${label} ends the intro at once, and cleanly`, async () => {
    ...
    // Playwright's own transport and screenshot latency, which under a full
    // parallel run on software GL has been over a second on its own.
    await page.addInitScript(() => {
    ...
    // A short fade, not a cut, and not the 3.9 seconds that were still to come.
